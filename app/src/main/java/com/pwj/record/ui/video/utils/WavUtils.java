package com.pwj.record.ui.video.utils;
...
public class WavUtils {
    ...
    public static void writeHeader(File file, byte[] header) {
        if (!RecordFileUtils.isFile(file)) {
            return;
        }
    ...
    private static byte[] getHeader(String wavFilePath) {
        ...
        try {
            ...
            if ((len = fis.read(b)) != size) {
                Log.e(TAG, String.format("读取失败 len: %s", len));
                return null;
            }
            bos.write(b, 0, len);
    ...
    public static long getWavDuration(String filePath) {
        if (!filePath.endsWith(RecordHelper.RecordFormat.WAV.getExtension())) {
            return -1;
        }
