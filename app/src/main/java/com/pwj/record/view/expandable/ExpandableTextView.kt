 ...
 * FileName:         ExpandableTextView
 * description:      description
 */
class ExpandableTextView @JvmOverloads constructor(context: Context, attributeSet: AttributeSet? = null, defStyleAttr: Int = 0) :
    AppCompatTextView(context, attributeSet, defStyleAttr) {

    private val ELLIPSIS_STRING = String(charArrayOf('\u2026'))
    ...
    private val mOpenSpannableStr: SpannableStringBuilder? = null
    ...
    fun setOriginalText(originalText: CharSequence) {
        ...
        mOpenSuffixStr = charSequenceToSpannable(originalText).toString()

        if (maxLines != -1) {
            val layout: Layout = createStaticLayout(tempText)
            mExpandable = layout.lineCount > maxLines
            if (mExpandable) { //拼接展开内容
                if (mCloseInNewLine) {
                ...
                var tempText2: SpannableStringBuilder = charSequenceToSpannable(mCloseSpannableStr!!).append(ELLIPSIS_STRING)
                if (mOpenSuffixSpan != null) {
                    tempText2.append(mOpenSuffixSpan)
                }
                //循环判断，收起内容添加展开后缀后的内容
                var tempLayout: Layout = createStaticLayout(tempText2)
                ...
                if (lastSpace >= 0 && originalText.length > lastSpace) {
                    ...
                    val offset: Int = hasEnCharCount(redundantChar) - hasEnCharCount(mOpenSuffixSpan!!) + 1
    ...
    private fun open() {
        if (hasAnimation) {
            val layout: Layout = createStaticLayout(mOpenSpannableStr!!)
    ...
    /** 执行展开动画  */
    private fun executeOpenAnim() { //创建展开动画
        if (mOpenAnim == null) {
            mOpenAnim = ExpandCollapseAnimation(this, mCLoseHeight, mOpenHeight)
            mOpenAnim!!.fillAfter = true
            mOpenAnim!!.setAnimationListener(object : AnimationListener {
                override fun onAnimationStart(animation: Animation) {
                    super@ExpandableTextView.setMaxLines(Int.MAX_VALUE)
                    text = mOpenSpannableStr
                }

                override fun onAnimationEnd(animation: Animation) { //  动画结束后textview设置展开的状态
    ...
    /** 执行收起动画  */
    private fun executeCloseAnim() { //创建收起动画
        if (mCloseAnim == null) {
            mCloseAnim = ExpandCollapseAnimation(this, mOpenHeight, mCLoseHeight)
            mCloseAnim!!.fillAfter = true
            mCloseAnim!!.setAnimationListener(object : AnimationListener {
                override fun onAnimationStart(animation: Animation) {}
    ...
    private fun updateCloseSuffixSpan() {
        ...
        mCloseSuffixSpan!!.setSpan(StyleSpan(Typeface.BOLD), 0, mCloseSuffixStr.length, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)
        if (mCloseInNewLine) {
   ...
   override fun setOnClickListener(l: OnClickListener?) {
    ...
    internal class ExpandCollapseAnimation(//动画执行view
